...
import os
import argparse
import tempfile # Keep for now, might be useful for other things later
import subprocess
import io
...
DEFAULT_CONVERT_EXTENSIONS = "wma,wmv,mp3,mp4,mkv,aac,flac,ogg,m4a,avi,mov,flv,mpeg,mpg,webm" # Added common non-wav types
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert audio files and transcribe them using Whisper."
    )
    parser.add_argument(
        "project_dir",
        type=Path,
        help=f"Path to the project directory. Expects a '{RAW_AUDIO_DIR_NAME}' subdirectory."
    )
    parser.add_argument(
        "--configuration",
        type=str,
        default=None,
        help="Path to the Whisper configuration JSON file."
    )
    parser.add_argument(
        "--run-description",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional tag to create a subfolder within the transcriptions directory."
    )
    parser.add_argument(
        "--no-recursive",
        action="store_false",
    ...
    if not raw_audio_path.is_dir():
        logging.error(f"'{RAW_AUDIO_DIR_NAME}' directory not found in {project_path}. Please create it and add audio files.")
        return
    ...
    if args.run_description:
        ...
        if not configurations:
            logging.error(f"No configuration found with description: '{args.run_description}'. Available descriptions in {config_file_path}:")
            # Log available descriptions from the originally loaded configurations if needed (requires re-reading or storing them)
            # For now, just error out.
            return
        logging.info(f"Filtered to 1 configuration based on --run-description '{args.run_description}' (out of {original_config_count}).")
    ...
    logging.info(f"Proceeding to transcribe {len(wav_files_for_transcription)} WAV files.")

    # --- Transcription Loop ---
    for configuration_obj in configurations: # Renamed to avoid conflict
        logging.info(f"Using Whisper Configuration: {configuration_obj}") # Assumes Configuration has __str__
        for wav_file_to_transcribe in wav_files_for_transcription:
            logging.info(f"Preparing to transcribe: {wav_file_to_transcribe.name}")
            ...
            original_relative_path_for_log = source_file_to_original_relative_path_map.get(str(wav_file_to_transcribe), "Unknown_Original_Path")

            logging.info(f"  Output base for transcription: {transcriptions_output_base}")
            logging.info(f"  Relative subdir for this file: {relative_dir_for_transcription}")
            logging.info(f"  Original source relative path (for logging): {original_relative_path_for_log}")
            ...
            if not args.dry_run:
                try:
                    ...
                    if args.enable_diarization:
                        ...
                        if not hf_token:
                            logging.warning("Diarization enabled but HF_TOKEN environment variable not set. Diarization might not work correctly.")
                        ...
                        logging.info("Speaker diarization enabled for this transcription.")
                    ...
                    if args.enable_diarization and hasattr(configuration_obj, 'config'):
                        ...
                        modified_config = Configuration(config_copy)
                        transcriber = modified_config
                    else:
                        transcriber = configuration_obj
                    ...
                    logging.info(f"Successfully submitted {wav_file_to_transcribe.name} for transcription with config.")
                    summary["transcriptions_submitted"] += 1
                except Exception as e:
                    logging.error(f"Error during transcription call for {wav_file_to_transcribe.name}: {e}")
                    logging.exception("Traceback:")
                    summary["transcription_errors"] += 1
            else:
                logging.info(f"[DRY RUN] Would transcribe {wav_file_to_transcribe.name} with config '{configuration_obj.config.get('description')}'")
                logging.info(f"[DRY RUN]   Output would be in a structure under: {transcriptions_output_base / relative_dir_for_transcription}")
                if args.enable_diarization:
                    logging.info(f"[DRY RUN]   Speaker diarization would be enabled")
                if not args.no_config_logs:
                    logging.info(f"[DRY RUN]   Would create config log file in the output directory")
                else:
                    logging.info(f"[DRY RUN]   Config log file creation is disabled")
