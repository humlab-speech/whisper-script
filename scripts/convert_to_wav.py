...
def convert_file(file, output_dir):
    output = os.path.join(
        output_dir, os.path.splitext(os.path.basename(file))[0] + ".wav"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert audio files to 16 kHz mono .wav"
    )
    parser.add_argument("input", help="Input file or directory")
    parser.add_argument(
        "--extensions",
        default="wma,wmv,mp3,mp4,mkv,aac,flac,ogg,m4a,wav,avi,mov,flv,mpeg,mpg,webm",
        help="File extensions to convert, separated by comma",
    )
